...
import { View, Text, TouchableOpacity, Image, StyleSheet, ActivityIndicator, ImageBackground, Alert, ScrollView, StatusBar } from 'react-native';
...
export default function Nutrition_Extraction_Multi() {
  ...
  const uploadImage = async (imageUri) => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to extract nutrition information: ' + (error.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const takePhoto = async () => {
    if (cameraRef.current) {
      try {
        ...
        // Not automatically analyzing after taking photo
        // Let user decide when to extract nutrition
      } catch (error) {
    ...
    <View style={styles.container}>
      <ImageBackground source={require('../../assets/background.png')} style={styles.backgroundImage}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          {/* Only show Extract button when image is available */}
          {(photo || image) && (
            <TouchableOpacity
              ...
              disabled={loading}
            >
              <Text style={styles.buttonText}>{loading ? 'Processing...' : 'Extract Nutrition'}</Text>
